...
DEFAULT_TIMEOUT = float(os.environ.get("SEARCH_MCP_TIMEOUT", "20"))

mcp = FastMCP("search") if FastMCP else None
...
def search(query: str, max_results: int = 8) -> dict[str, Any]:
    ...
    results = [_normalize_search_result(item) for item in raw_results[: max(1, int(max_results))]]
    return {"ok": True, "backend": "searxng", "query": query, "results": results}


@_tool
def scrape_url(url: str, only_main_content: bool = True) -> dict[str, Any]:
    ...
    try:
        data = _json_request(endpoint, payload)
    except Exception as exc:
...
def main() -> None:
    if mcp is None:
        raise SystemExit("The mcp package is required to run the search MCP server.")
    mcp.run()
