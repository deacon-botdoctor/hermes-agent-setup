...
def check_gateway_state():
    ...
    return {
        ...
        'status':'pass' if ok else 'fail',
        'detail':('pids='+','.join(proc.stdout.split()[:5]))
        if ok else 'no process matched hermes home',
    }

def gateway_runtime_binding():
    candidates = [HERMES/'state/gateway-state.json', HERMES/'state/gateway_state.json', HERMES/'gateway_state.json']
    data = {}
    state_path = None
...
def check_immersion_quality():
    ...
    notifications = _top_level_block(text, 'notifications')
    ...
    required_display = {
        ...
        'context_pressure_notifications': False,
    ...
    required_notifications = {
        'compression_events': False,
        'session_reset_events': False,
        'provider_fallback_events': False,
        'provider_warning_events': False,
        'tool_step_intermediate': False,
    }
    drift = []
    ...
    for key, expected in required_notifications.items():
        actual = _scalar_from_block(notifications, key)
        if actual != expected:
            drift.append(f'notifications.{key}={actual!r}->{expected!r}')
...
def check_agent_probe():
    ...
    if not isinstance(main, dict):
        matches = [
            body for body in gateways.values()
            if isinstance(body, dict)
            and Path(str(body.get('hermes_home') or '')).expanduser() == HERMES
        ]
...
def _is_synthetic_compaction_reference(role, content):
    return str(role or '') == 'assistant' and str(content or '').startswith(
        ('[PRIOR CONTEXT', '[CONTEXT COMPACTION]')
    )
...
def check_credential_friction_recent():
    ...
    try:
        ...
        rows = [
            row
            for row in rows
            if not _is_synthetic_compaction_reference(row[0], row[3])
        ]
        tool_fail_hits = [
            row
            for row in rows
            if str(row[0]) == 'tool'
            and _is_explicit_tool_credential_error(row[1], row[3])
        ]
        ...
        return {
            ...
            'status': status,
            'detail':(
                f'hits={len(rows)} explicit_tool_errors={len(tool_fail_hits)} samples='
                + ' | '.join(samples)
            ),
        }
    except Exception as e:
...
CHECK_METADATA = {
    ...
    'credential_friction_recent': {
        ...
        'recommended_action': (
            'Classify whether the missing credential is expected, a Composio '
            'connection gap, or stale tool advertising.'
        ),
    },
    'advertised_tool_env': {
        ...
        'recommended_action': (
            'Either provide the referenced env key or stop advertising the '
    ...
    'tool_readiness': {
        ...
        'recommended_action': (
            'Refresh the local tool readiness probe and repair broken core tools '
...
def check_disk():
    ...
    try:
        ...
        pct=0
    return {'name':'disk','status':'fail' if pct>=95 else 'pass','detail':f'used={used}'}
...
def main():
    ...
    try:
        ...
        (HERMES/'logs/local-selfcheck.log').open('a', encoding='utf-8').write(
            f"{payload['checked_at']} status={payload['status']} "
