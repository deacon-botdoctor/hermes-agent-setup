...
GATEWAY_NOISY_REPLACEMENT = (
    ...
    + """    # HERMES_SILENT_AUTO_CONTEXT_LIFECYCLE_v1
...
STARTUP_FAILED_NATIVE_021_GUARD = """        if (
            decision is not None
            and not decision.durable_accepted
...
STARTUP_FAILED_NATIVE_021_FINAL = """        if (
            decision is not None
            and not decision.durable_accepted
            and not decision.response
        ):
            # HERMES_DRAIN_ADMISSION_FAILURE_VISIBILITY_v1 — transports that
            # can retry must do so; long-poll transports need a visible resend
            # request because their update offset advances after this callback.
            if event.retry_transport_on_admission_failure:
                raise RuntimeError("startup-gate admission requires transport retry")
            decision = _StartupGateDecision(
                "I couldn't safely save that message during a restart. "
...
DRAIN_FAILURE_TEST_021_OLD = (
    '    warning = adapter._send_with_retry.await_args.kwargs["content"]'
    "  # ty:ignore[unresolved-attribute]\n"
    '    assert "could not confirm completion" in warning\n'
    '    assert "did not retry" in warning\n'
    "    assert event.text not in warning\n"
    '    assert adapter.deleted == [(event.source.chat_id, "ack-uncertain")]\n'
)
DRAIN_FAILURE_TEST_021_NEW = """    # HERMES_SILENT_DRAIN_NOTICE_DELIVERY_v2 — ambiguity is recorded and
    # retired without emitting an empty or control-plane client message.
    adapter._send_with_retry.assert_not_awaited()  # ty:ignore[unresolved-attribute]
    assert adapter.deleted == [(event.source.chat_id, "ack-uncertain")]
"""
...
RESTART_PERSIST_RESULT_OLD = '    assert "saved" in await persist_task  # ty:ignore[unsupported-operator]\n'
...
RELAY_APPROVAL_LATEST_OLD = """                if not count:
                    label = "⌛ Approval expired — no command was waiting."
                # Acknowledge in-channel (the connector's prompt message can't
                # be edited cross-platform yet — edit support varies; a short
                # confirmation preserves the audit trail the native edit gives).
...
RELAY_APPROVAL_LATEST_NEW = """                # HERMES_CLIENT_CONVERSATION_CONTINUITY_v1
                if count:
                    # Fire-and-forget: we are ON the read loop here (see
                    # _send_lifecycle_ack) — awaiting the send self-deadlocks the
                    # transport for the full outbound timeout.
...
TEAMS_EXPIRED_NEW = """        if not has_blocking_approval(session_key):
            ...
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value=""),
            )
"""
...
def _replace_exact(
    ...
    *,
    ...
    if found != count:
        raise RuntimeError(f"{label} anchor missing or ambiguous: expected {count}, found {found}")
...
def patch_continuity_gateway_text(source: str) -> str:
    ...
    if DRAIN_SILENT_DELIVERY_MARKER not in patched:
        ...
        if STARTUP_FAILED_SILENT_GUARD_OLD in patched:
            patched = _replace_exact(
                patched,
        ...
        elif STARTUP_FAILED_NATIVE_021_GUARD in patched:
            patched = _replace_exact(
                patched,
                STARTUP_FAILED_NATIVE_021_GUARD,
                STARTUP_FAILED_NATIVE_021_FINAL,
                label="native 0.21 failed startup admission",
            )
        else:
            raise RuntimeError(
                "silent failed startup admission anchor missing or ambiguous: "
                "expected one supported lifecycle seam"
            )
    ...
    if remaining:
        raise RuntimeError("client-visible startup recovery text remained: " + ", ".join(remaining))
    if patched.count(STARTUP_RESPONSE_GUARD) != 1:
        raise RuntimeError("startup response truthiness guard missing or ambiguous")
    return patched
...
def patch_restart_test_text(source: str) -> str:
    ...
    if DRAIN_SILENT_DELIVERY_MARKER not in patched:
        saved_assert_count = patched.count(RESTART_SAVED_ASSERT_OLD)
        if saved_assert_count not in {3, 4}:
            raise RuntimeError(
                "silent startup saved assertions anchor missing or ambiguous: "
                f"expected 3 or 4, found {saved_assert_count}"
            )
        patched = _replace_exact(
            patched,
            RESTART_SAVED_ASSERT_OLD,
...
def patch_drain_consumer_test_text(source: str) -> str:
    ...
    if DRAIN_SILENT_DELIVERY_MARKER not in patched:
        if DRAIN_FAILURE_TEST_OLD in patched:
            patched = _replace_exact(
                patched,
                DRAIN_FAILURE_TEST_OLD,
            ...
            patched = _replace_exact(
                patched,
                DRAIN_FAILURE_TEST_021_OLD,
                DRAIN_FAILURE_TEST_021_NEW,
                label="silent 0.21 ambiguous drain completion test",
            )
        patched = _replace_exact(
            patched,
...
def patch_relay_approval_text(source: str) -> str:
    ...
    matches = [(old, new) for old, new in candidates if old in source]
    if len(matches) != 1:
        raise RuntimeError(f"relay expired approval anchor missing or ambiguous: expected 1, found {len(matches)}")
    old, new = matches[0]
    return source.replace(old, new, 1)


def _approval_uncertainty(source: str, platform: str) -> str:
    pairs = {
        ...
        "discord": (('                logger.error("Failed to resolve gateway approval from button: %s", exc)\n                count = 0\n', '                logger.error("Failed to resolve gateway approval from button: %s", exc)\n                count = None\n'), ('            # Update the embed with the decision. A stale callback removes\n            # controls without adding expiry text to the client transcript.\n            embed = interaction.message.embeds[0] if interaction.message.embeds else None\n            if embed:\n                if count:\n                    embed.color = color\n                    embed.set_footer(\n                        text=f"{label} by {interaction.user.display_name}"\n                    )\n                else:\n                    # HERMES_CLIENT_CONVERSATION_CONTINUITY_v1\n                    embed.remove_footer()\n', '            # Update the embed with the decision. A stale callback removes\n            # controls without adding expiry text to the client transcript.\n            embed = interaction.message.embeds[0] if interaction.message.embeds else None\n            if embed:\n                if count is None:\n                    embed.color = discord.Color.dark_grey()\n                    embed.set_footer(text=\'Approval could not be confirmed. Check the command status before retrying.\')\n                elif count:\n                    embed.color = color\n                    embed.set_footer(\n                        text=f"{label} by {interaction.user.display_name}"\n                    )\n                else:\n                    # HERMES_CLIENT_CONVERSATION_CONTINUITY_v1\n                    embed.remove_footer()\n')),
    }[platform]
    for before, after in pairs:
        if after not in source:
            source = _replace_exact(source, before, after, label=platform + " approval uncertainty")
    return source
...
def patch_slack_approval_text(source: str) -> str:
    ...
    return _approval_uncertainty(patched, 'slack')
...
def patch_discord_approval_text(source: str) -> str:
    ...
    return _approval_uncertainty(patched, 'discord')
...
_NATIVE_D363_PAYLOAD = Path(__file__).resolve().parents[1] / "payloads" / "silent-context-lifecycle-d363-v1"
...
def patch_silent_context_lifecycle_v1(
    ...
    *,
    ...
    if _is_native_d363(hermes_dir):
        return _patch_native_d363(hermes_dir, dry_run=dry_run)
    gateway_path = _target(hermes_dir, "gateway", "run.py")
    targets = {
        _target(hermes_dir, "agent", "conversation_compression.py"): patch_completion_text,
        gateway_path: lambda source: patch_continuity_gateway_text(patch_gateway_text(source)),
        _target(
            hermes_dir,
            "tests",
            "gateway",
    ...
    for path in changed:
        backup = path.with_suffix(path.suffix + f".bak-{stamp}-pre-silent-context-lifecycle-v1")
